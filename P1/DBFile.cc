...
void DBFile::MoveFirst() {
	  ...
	  this->file->GetPage(this->readPage,
			  1);  // TODO: check off_t type,  void GetPage
			       // (Page *putItHere, off_t whichPage)
	  this->readPage->GetFirst(
	      this->current);  // check if this->readPage is pointer
			   //}
}
...
int DBFile::GetNext(Record &fetchme) {
	  ...
	  if (endOfFile != 1) {
		    ...
		    if (result ==
		        0) {  // int GetFirst (Record *firstOne), check type

			      pageIndex++;

			      if (pageIndex >=
				this->file->GetLength() - 1) {
				        endOfFile = 1;
			      }
...
int DBFile::GetNext(Record &fetchme, CNF &cnf, Record &literal) {
	  ...
	  while (result1 == 0 && result2 != 0) {
		    result2 = this->GetNext(
		        fetchme);  // requires reference..check if is right
		    result1 = compare.Compare(
		        &fetchme, &literal,
		        &cnf);  // int Compare(Record *left, Record
			      // *literal, CNF *myComparison); is this
			      // right
	  }
