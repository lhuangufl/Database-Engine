...
public:
	...
	template <typename T>
	T* GetAttVal(int att_idx) {
		int attLoc = ((int*)(bits)) [att_idx + 1];
		return (T*)(bits + attLoc);
		/*
		switch (att_type) {
			...
			case String:{
				return std::string((char*)(bits + attLoc));
			}
			default:{
				std::cerr << "[Error] In function Record::GetAttVal(), invalid attribute type" << std::endl;
				exit(1);
			}
		}*/
	}
