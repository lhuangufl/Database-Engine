...
void test1() {
	...
	if (proc && res) {
		...
		if (x < 3) {
			...
			if (dbfile.Create(rel->path(), sorted, &startup)) {
				cout << "DBFileSort Created Successfully!!" << endl;
			}
			if (dbfile.Close()) {
				cout << "DBFileSort Closed Successfully!@" << endl;
			}
			//=======
			proc = add_data(tblfile, lrand48() % (int)pow(1e3, x) + (x - 1) * 1000, res);
			tot += proc;
			cout << "Finish Adding Data back in test, proc = " << proc << endl;
			if (proc)
...
void test2() {
	...
	// 	Page bufferPage;
	// 	Record temp;
	// 	cout << "Scan file " << rel->path() << endl;
	// 	dbfile.Open(1, "tmp.bigQ");
	// 	int i = 0, cnt = 0;
	// 	cout << "dbfile.GetLength() = " << dbfile.GetLength() << endl;
	//
	// 	while (i < dbfile.GetLength() - 1) {
	// 		dbfile.GetPage(&bufferPage, i);
	// 		while (bufferPage.GetFirst(&temp)) {
	// 			temp.Print(rel->schema());
	// 			cnt++;
	// 			cout << "cnt ->" << cnt << endl;
	// 		}
